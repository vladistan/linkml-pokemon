...
def process_mermaid_blocks(html_content):
    ...
    patterns = [
        (r'<div class="highlight"><pre><span></span><code><span class="n">classDiagram</span>(.*?)</code></pre></div>', 'classDiagram'),
        (r'<div class="highlight"><pre><span></span><code>erDiagram\n(.*?)</code></pre></div>', 'erDiagram')
    ]
    ...
    for pattern, diagram_type in patterns:
        result = re.sub(pattern, lambda m: replace_mermaid(m, diagram_type), result, flags=re.DOTALL)
...
class MermaidMarkdownHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def serve_markdown(self):
        ...
        try:
            ...
            if not datadict_path.exists():
                self.send_error(404, "datadict.md not found. Run 'just gen-project' first.")
                return
...
def main():
    ...
    print(f"🚀 LinkML Pokemon Data Dictionary Server")
    print(f"📊 Server running at: http://localhost:{port}")
    print(f"📈 Full Mermaid diagram support enabled!")
    print(f"📁 Serving: project/datadict.md")
    ...
    try:
        with socketserver.TCPServer(("localhost", port), MermaidMarkdownHandler) as httpd:
            httpd.serve_forever()
        ...
        if "Address already in use" in str(e):
            ...
            print(f"💡 Try a different port: uv run --group dev python mermaid_server.py {port + 1}")
        else:
